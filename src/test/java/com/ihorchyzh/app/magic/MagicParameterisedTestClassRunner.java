 ...
 */
public class MagicParameterisedTestClassRunner {

    private Map<MagicTestMethod, MagicParameterisedTestMethodRunner> parameterisedMethods = new HashMap<MagicTestMethod, MagicParameterisedTestMethodRunner>();
    private Map<FrameworkMethod, MagicTestMethod> testMethods = new HashMap<FrameworkMethod, MagicTestMethod>();
    private List<MagicTestMethod> testMethodsList;
     ...
     * @param testClass
     */
    public MagicParameterisedTestClassRunner(TestClass testClass) {
    ...
    private void computeTestMethods(TestClass testClass) {
        testMethodsList = MagicTestMethod.listFrom(testClass.getAnnotatedMethods(Test.class), testClass);
    }

    private void fillTestMethodsMap() {
        for (MagicTestMethod testMethod : testMethodsList)
    ...
    private void cacheMethodRunner(MagicTestMethod testMethod) {
        if (!parameterisedMethods.containsKey(testMethod))
            parameterisedMethods.put(testMethod, new MagicParameterisedTestMethodRunner(testMethod));
    }
    ...
    public Statement parameterisedMethodInvoker(FrameworkMethod method, Object testClass) {
        MagicTestMethod testMethod = testMethods.get(method);
    ...
    private Statement buildMethodInvoker(FrameworkMethod method, Object testClass, MagicTestMethod testMethod) {
        MagicParameterisedTestMethodRunner parameterisedMethod = parameterisedMethods.get(testMethod);

        return new MagicInvokeParameterisedMethod(
     ...
     */
    public boolean shouldRun(MagicTestMethod testMethod) {
    ...
    public void runParameterisedTest(MagicTestMethod method, Statement methodInvoker, RunNotifier notifier) {
     ...
     */
    public MagicTestMethod testMethodFor(FrameworkMethod method) {
